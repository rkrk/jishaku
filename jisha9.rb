...
get '/jisha/:no/send' do 
	request.env.map { |e| e.to_s + "\n" }
	@jisha_no = params[:no].to_i
	@jisha = @@jisha[@jisha_no][0]
	erb :send_telegram
end

post '/jisha/:no/send/single' do
	@jisha = @@jisha[params[:no].to_i][0]
	@jisha.send_q  << params[:single_telegram].chomp
	redirect back
end

post '/jisha/:no/send/batch' do
	@jisha = @@jisha[params[:no].to_i][0]
	# @jisha.send_q  << params[:single_telegram].chomp
	redirect back
end

get '/jisha/:no/rtlog' do 
	request.env.map { |e| e.to_s + "\n" }
	erb :rtlog
end
